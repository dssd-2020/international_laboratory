...
def start_processing(request):
    ...
    """
    data = request.data
    logging.info('Se solicitó iniciar el procesamiento del proyecto %s en el caso %s', data['project'], data['processInstanceId'])
    try:
        ...
        return JsonResponse({'error': 'No existen protocolos para este proyecto.', 'status': status.HTTP_400_BAD_REQUEST})
    except Exception as e:
        logging.debug('Excepcion: %s', str(e))
        return JsonResponse({'error': str(e), 'status': status.HTTP_400_BAD_REQUEST})
...
def get_result_by_protocol(protocol_project, activities_checked):
    ...
    logging.info('Los puntos necesarios eran %s, los puntos obtenidos fueron %s', protocol_project.protocol.points, points)
